function n = navier_q1(xy,ev,tpde,flowsol,tout)
...
if nargin < 5, tout = 1; end
if nargin < 4, flowsol = tpde; tpde = 1; end % Hack for one-phase case
nngpt=4; 
x=xy(:,1); y=xy(:,2);
...
%  evaluate derivatives etc
		 ...
		 for j = 1:4
            for i = 1:4               
				ne(:,i,j)  = ne(:,i,j)  + wght*tpde.*u_x(:).*phi(:,i).*dphidx(:,j);
                ne(:,i,j)  = ne(:,i,j)  + wght*tpde.*u_y(:).*phi(:,i).*dphidy(:,j);
             end
	    end
%
