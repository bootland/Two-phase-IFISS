function [a,f,mump,rhomp] = fpsetup_q1_2s(xy,xyp,mv,mp,flowsol,tpve,tpde,domain)
%FPSETUP_Q1_2S Q1 pressure convection-diffusion preconditioner matrices
...
%          domain     domain index
%   output
%          Ap         viscosity scaled Q1 scalar diffusion matrix
%          Fp         scaled Q1 scalar convection-diffusion matrix
%          muMp       viscosity inversly scaled Q1 mass matrix
%          rhoMp      density scaled Q1 mass matrix
%
...
ae = zeros(nel,4,4); % ape = zeros(nel,4,4);
fe = zeros(nel,4,4); de = zeros(nel,4,4); mumpe = zeros(nel,4,4); rhompe = zeros(nel,4,4);
...
for igpt = 1:nngpt
    ...
    for j = 1:4
        for i = 1:4
            ...
            ae(:,i,j)  = ae(:,i,j)  + wght*tpve.*dphidy(:,i).*dphidy(:,j).*invjac(:);zz
%             ape(:,i,j)  = ape(:,i,j)  + wght*tpdve.*dphidx(:,i).*dphidx(:,j).*invjac(:);
%             ape(:,i,j)  = ape(:,i,j)  + wght*tpdve.*dphidy(:,i).*dphidy(:,j).*invjac(:);
            fe(:,i,j)  = fe(:,i,j)  + wght*tpde.*u_x(:).*phi(:,i).*dphidx(:,j);
            fe(:,i,j)  = fe(:,i,j)  + wght*tpde.*u_y(:).*phi(:,i).*dphidy(:,j);
            ...
            rhompe(:,i,j) = rhompe(:,i,j) + wght*tpde.*phi(:,i) .*phi(:,j) .*jac(:);
            mumpe(:,i,j) = mumpe(:,i,j) + (wght./tpve).*phi(:,i) .*phi(:,j) .*jac(:);
        end
    end
    %
...
epe = flow_h.*flow_l2/2; epe = epe/tpve;
epw = flow_l2;
%
...
if any(expe),
    ...
    if ssupg==1
        ...
        fe = fe + de;
        disp('Streamline diffusion added')
    end
end
%
...
%         ap = ap + sparse(nrow,ncol,ape(:,krow,kcol),np,np);
        f = f + sparse(nrow,ncol,fe(:,krow,kcol),np,np);
        rhomp = rhomp + sparse(nrow,ncol,rhompe(:,krow,kcol),np,np);
