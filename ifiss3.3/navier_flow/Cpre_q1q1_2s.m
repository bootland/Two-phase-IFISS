function [Cp1,Cp2] = Cpre_q1q1_2s(xy,ev,tpve)
%CPRE_Q1Q1_2S generate stabilization matrices for Q1-Q1 
...
for igpt = 1:nngpt
   ...
   for j = 1:4
      for i = 1:4
          cpe1(:,i,j) = cpe1(:,i,j) + .25 * (wght./tpve).*(phi(:,i)-0.25) .*(phi(:,j)-0.25) ;
          cpe2(:,i,j) = cpe2(:,i,j) + (16*jac(:)) .\ ((wght./tpve).*(phi(:,i)-0.25) .*(phi(:,j)-0.25));
      end
   end
