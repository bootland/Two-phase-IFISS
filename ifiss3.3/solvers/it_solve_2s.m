...
%
   ...
   if nlmethod==0,
      ...
      if qmethod>1,
         ...
         afun_par = struct('Afun','a_nst','F',Anst,'B',Bst,'D',-beta*muC);
      end
   else      
...
%       J = tpvd*A + [N + Nxx, Nxy; Nyx, N + Nyy];
      ...
      if qmethod>1, 
         ...
         afun_par = struct('Afun','a_nst','F',Jnst,'B',Bst,'D',-beta*muC);
      end
   end
%
   % select Krylov subspace method and set parameters 
   if testrun ~= 1
       ...
       tol = default('stopping tolerance? (default 1e-6)',1e-6);
       maxit = default('maximum number of iterations? (default 1000)',1000);
   else
       itmeth = 1;
       tol = 1e-6;
       maxit = 2000;
   ...
   if testrun ~= 1
       ...
       fprintf('   6  self-p (Sp)\n');
       fprintf('   7  scaled pressure mass matrix (sMp)\n');
       precon = default('default is least-squares commutator',3);
       if precon>0,
           query = 'ideal / AMG iterated  preconditioning? 1/2 (default AMG)';
           precon_format = default(query,2);
           if (precon_format>2 ),
       ...
       if precon == 3 && (qmethod == 0 || qmethod == 2)
           lapl = default('Discrete Laplacian: BQ^-1B^T / Ap? 1/2 (default BQ^-1B^T)',1);
       elseif precon == 2 && (qmethod == 0 || qmethod == 2)
           lapl = default('Discrete Laplacian: BQ^-1B^T / Ap? 1/2 (default Ap)',2);
       elseif precon == 7
           mmm = default('Mass matrix mode? diag/lumped diag/full 1/2/3  (default diag)',1);
       else
           if precon == 2
               lapl = 2;
           else
...
%
   ...
   if precon==2 || precon==8 || (precon==3 && lapl==2) ,      %%%%  Fp: need construction of Ap and Fp blocks
      if precon==3
          tpve = 1./tpve;
          tpde = 1./tpde;
      end
      ...
      elseif qmethod==2
         ...
         [Ap,Fp,muMp,rhoMp] = fpsetup_q1_2s(xy,xyp,mv,mp,flowsol,tpve,tpde,domain); 
      elseif qmethod==3
      ...
      elseif qmethod==0
         %% set up Fp operator for Q1 approximation
% %          [Ap,Fp] = fpsetup_q1_2(xy,xyp,ev,ev,flowsol,tpve,tpde,domain);
         [Ap,Fp,muMp,rhoMp] = fpsetup_q1_2s(xy,xyp,ev,ev,flowsol,tpve,tpde,domain); 
      end
      if deltat ~= 0
          Fp = Fp + rhoMp/deltat;
      end
   ...
   elseif precon==2 && lapl==1
      if qmethod==0
         imuG = q1_scaled_velocity_mass_matrix(xy,ev,1./tpve);
      elseif qmethod==2
         imuG = q2_scaled_velocity_mass_matrix(xy,xyp,mv,mp,1./tpve);
      else
         error('Not yet implemented!')
      end
   elseif precon==3
      if qmethod==0
         muG = q1_scaled_velocity_mass_matrix(xy,ev,tpve);
      elseif qmethod==2
         muG = q2_scaled_velocity_mass_matrix(xy,xyp,mv,mp,tpve);
      else
         error('Not yet implemented!')
      end
   elseif precon==3.1
      muG = spdiags(diag(afun_par.F),0,nnv,nnv);
   elseif precon==3.2
      muG = spdiags(max(afun_par.F,[],2),0,nnv,nnv);
   ...
   elseif precon==7 % scaled pressure mass matrix
      if qmethod==0
         muMp = q1_scaled_pressure_mass_matrix(xy,xyp,ev,ev,1./tpve);
      elseif qmethod==2
         muMp = q1_scaled_pressure_mass_matrix(xy,xyp,mv,mp,1./tpve);
      else
         error('Not yet implemented!')
      end
   end
   matrix_setup_time = toc(setup);
   if precon_format==1, 
   ...
   % in preconditioner
      ...
      elseif precon==2,   %%%%  pressure commutator (Fp) 
         ...
         if qmethod<2
            mfun_par = struct('Mfun','m_fp','Fp',Fp,'Ap',Ap,'Mp',muMp,'domain',domain,'n_null',n_null);
         else
            if lapl==1
                iGdiag=spdiags(diag(imuG),0,nnv,nnv);
                BBt = Bst*(iGdiag\Bst');
                mfun_par = struct('Mfun','m_fp','Fp',Fp,'Ap',BBt,'Mp',muMp,'domain',domain,'n_null',n_null);
            else
                mfun_par = struct('Mfun','m_fp','Fp',Fp,'Ap',Ap,'Mp',muMp,'domain',domain,'n_null',n_null);
            end
         end
      elseif floor(precon)==3    %%%% (scaled) least squares (BFBt)
         fprintf('ideal least-squares commutator preconditioning ...\n')
         if qmethod>1
            if lapl==1
                mfun_par = struct('Mfun','m_xbfbt','domain',domain,'G',muG,'n_null',n_null);
            else
                mfun_par = struct('Mfun','m_xbfbt_ap','domain',domain,'G',muG,'Ap',Ap,'n_null',n_null);
            end
         elseif qmethod==0,
			[Cp1,Cp2] = Cpre_q1q1_2s(xy,ev,tpve);
            if lapl==1
                mfun_par = struct('Mfun','m_sxbfbt_2s','domain',domain,'G',muG,...
							  'Cp1',Cp1,'Cp2',Cp2,'n_null',n_null);
            else
                mfun_par = struct('Mfun','m_sxbfbt_2s_ap','domain',domain,'G',muG,...
							  'Ap',Ap,'Cp2',Cp2,'n_null',n_null);
            end
		 elseif qmethod==1,
			[Cp1,Cp2] = Cpre_q1p0(xy,xyp,ev,domain);
			Cp1=beta*Cp1; Cp2=beta*Cp2;
            mfun_par = struct('Mfun','m_sxbfbt','domain',domain,'G',muG,'Q',Q,...
      ...
      elseif precon==5     %%%% precon==5, boundary-adjusted least squares 
      ...
      elseif precon==6,     %%%% precon==6, selfp 
          fprintf('ideal selfp preconditioning ...\n');
          mfun_par = struct('Mfun','m_selfp','domain',domain,'n_null',n_null);
      elseif precon==7      %%%% precon==7, scaled mass matrix
         fprintf('ideal scaled mass matrix preconditioning ...\n');
         if mmm==3
             mfun_par = struct('Mfun','m_Mp','Mp',muMp,'domain',domain,'n_null',n_null);
         elseif mmm==2
             mfun_par = struct('Mfun','m_Mp','Mp',spdiags(muMp*ones(np,1),0,np,np),'domain',domain,'n_null',n_null);
         else
             mfun_par = struct('Mfun','m_Mp','Mp',spdiags(diag(muMp),0,np,np),'domain',domain,'n_null',n_null);
         end
      else %%%% precon==8 newcom
          fprintf('ideal new commutated preconditioning ...\n');
          if qmethod>1
            mfun_par = struct('Mfun','m_xnewcom','domain',domain,'Mp',muMp,'A',L,'n_null',n_null);
          else
              error('Element spaces not yet catered for\n');
          end
%       elseif precon==8,
%           fprintf('ideal stability matricx preconditioning ...\n');
%           mfun_par = struct('Mfun','m_stabmat','domain',domain,'n_null',n_null);
      end
      matrix_setup_time = toc(setup) + matrix_setup_time;
%
   else
 ...
 %    compute new MG data or reload existing data?
      ...
      elseif precon==2      %%% AMG iterated Fp
         ...
         if lapl==2
            ...
            mg_setup_time = mg_setup_time + toc(mg_setup);
         else
            setup = tic;
            iGdiag=spdiags(diag(imuG),0,nnv,nnv);
            BBt = Bst*(iGdiag\Bst');
            ...
            mg_setup = tic;
            if n_null>0,
                Apx = pressurebc(BBt,n_null);
                amg_gridA = amg_grids_setup(Apx);
            else
                amg_gridA = amg_grids_setup(BBt);
            end
            mg_setup_time = mg_setup_time + toc(mg_setup);
         end
         ...
         if lapl==2
            mfun_par = struct('Mfun','m_fp_amgz','Fp',Fp,'Ap',Ap,'Mp', ...
                              muMp,'domain',domain,'n_null',n_null);       
         else
             mfun_par = struct('Mfun','m_fp_amgz','Fp',Fp,'Ap',BBt,'Mp', ...
                              muMp,'domain',domain,'n_null',n_null);
         end
      elseif floor(precon)==3, %%% AMG iterated scaled BFBt 
         fprintf('AMG iterated LSC preconditioning ...\n')
         setup = tic;
         Gdiag=spdiags(diag(muG),0,nnv,nnv);
         if qmethod>1,
            if lapl==1
                if n_null>0,
         ...
         if qmethod>1, 
            ...
            mfun_par = struct('Mfun','m_sxbfbt_2s_amgz','domain',domain,...
                              'Gdiag',Gdiag,'Cp2',Cp2,'n_null',n_null); 
         end
      ...
      elseif precon==6,     %%%% precon==6, selfp 
         fprintf('AMG iterated selfp preconditioning ...\n');
         setup = tic;
         S = (afun_par.B)*spdiags(1./diag(afun_par.F),0,nnv,nnv)*(afun_par.B') - afun_par.D;
         matrix_setup_time = toc(setup) + matrix_setup_time;
         mg_setup = tic;
         if n_null>0,  
            Sx = pressurebc(S,n_null); 
            amg_gridA = amg_grids_setup(Sx);
         else
            amg_gridA = amg_grids_setup(S); 
         end
         fprintf('selfp setup done.\n')
         if damps==1
            fprintf('point damped Jacobi smoothing on finest level ..\n')
            smoother_paramsA = amg_smoother_params(amg_gridA, 'PDJ', 2);
         elseif damps==0
         ...
         amg_smootherA = amg_smoother_setup(amg_gridA, smoother_paramsA);
         mg_setup_time = mg_setup_time + toc(mg_setup);
         mfun_par = struct('Mfun','m_selfp_amgz','domain',domain,'n_null',n_null);
      else     %%%% precon==7, scaled mass matrix 
         fprintf('AMG iterated scaled mass matrix preconditioning ...\n');
         setup = tic;
         if mmm==3
             sMp = muMp;
         elseif mmm==2
             sMp = spdiags(muMp*ones(np,1),0,np,np);
         else
             sMp = spdiags(diag(muMp),0,np,np);
         end
         matrix_setup_time = toc(setup) + matrix_setup_time;
         mg_setup = tic;
         if n_null>0,  
            sMpx = pressurebc(sMp,n_null); 
            amg_gridA = amg_grids_setup(sMpx);
         else
            amg_gridA = amg_grids_setup(sMp); 
         end
         fprintf('Scaled mass matrix setup done.\n')
         if damps==1
            fprintf('point damped Jacobi smoothing on finest level ..\n')
            smoother_paramsA = amg_smoother_params(amg_gridA, 'PDJ', 2);
         elseif damps==0
         ...
         amg_smootherA = amg_smoother_setup(amg_gridA, smoother_paramsA);
         mg_setup_time = mg_setup_time + toc(mg_setup);
         mfun_par = struct('Mfun','m_sMp_amgz','domain',domain,'n_null',n_null);
      end
   end
%   
%
...
%       [x_it,flag,iter,resvec] = gmres_r_plot(afun_par,mfun_par,nlres,params,x0,qmethod,xns,By,Bx,A,xy,xyp,x,y,bound,spc);
   elseif itmeth==2, %BiCGSTAB(ell)
      fprintf('BiCGSTAB(%2i) iteration ...\n',ell);
