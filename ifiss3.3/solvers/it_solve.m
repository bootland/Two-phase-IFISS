...
%
   ...
   fprintf('   6  selfp (Sp)\n');
   fprintf('   7  scaled mass matrix (sMp)\n');
...
%
   ...
   % in preconditioner
      ...
      elseif precon==6,     %%%% precon==6, selfp 
         ...
         mmm = default('Mass matrix mode? full/diag/lumped diag 1/2/3  (default full)',1);
         if mmm==1
             mfun_par = struct('Mfun','m_sMp','Mp',Q,'viscosity',viscosity,'domain',domain,'n_null',n_null);
         elseif mmm==2
             mfun_par = struct('Mfun','m_sMp','Mp',spdiags(diag(Q),0,np,np),'viscosity',viscosity,'domain',domain,'n_null',n_null);
         else
             mfun_par = struct('Mfun','m_sMp','Mp',spdiags(Q*ones(np,1),0,np,np),'viscosity',viscosity,'domain',domain,'n_null',n_null);
         end
      end
%
   else
 ...
 %    compute new MG data or reload existing data?
      ...
      elseif precon==6,     %%%% precon==6, selfp 
         ...
         mmm = default('Mass matrix mode? full/fiag/lumped diag 1/2/3  (default full)',1);
         if mmm==1
             sQ = Q/viscosity;
         elseif mmm==2
             sQ = spdiags(diag(Q)/viscosity,0,np,np);
         else
             sQ = spdiags((Q*ones(np,1))/viscosity,0,np,np);
         end
         if n_null>0,  
            sQx = pressurebc(sQ,n_null); 
            amg_gridA = amg_grids_setup(sQ);
         else
            amg_gridA = amg_grids_setup(sQ); 
         end
