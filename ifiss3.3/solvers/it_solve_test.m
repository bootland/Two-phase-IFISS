...
%          [Ap,Fp] = fpsetup_q1(xy,xyp,mv,mp,flowsol,viscosity,domain);
         [Ap,Fp] = fpsetup_q1_2(xy,xyp,mv,mp,flowsol,spdiags(tpvh,0,length(tpvh),length(tpvh)),tpve,domain); 
      elseif qmethod==3
...
%          mfun_par = struct('Mfun','m_sMp','Mp',Q,'viscosity',viscosity,'domain',domain,'n_null',n_null);
         mfun_par = struct('Mfun','m_sMp2p','Mp',Q,'tpv',spdiags(tpvh,0,length(tpvh),length(tpvh)),'domain',domain,'n_null',n_null);
