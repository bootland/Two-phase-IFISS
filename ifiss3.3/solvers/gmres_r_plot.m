function [x, flag, total_iters, error] = gmres_r_plot(aparams,mparams, b, params, x0, qmethod,xns,By,Bx,A,xy,xyp,XX,YY,bound,spc)
...
%
    ...
    % Additional flow plot (slow!)
    ynew=h(1:k,1:k)\g(1:k);
    x_it_old = x_it;
    x_it = x0 + feval(mfun, v(1:n,1:k)*ynew, aparams,mparams);
    xnew = xns-x_it;
    figure(98), semilogy(0:k,error), drawnow, shg
    flowplot(qmethod,xnew,By,Bx,A,xy,xyp,XX,YY,bound,spc,99); drawnow;
%     flowplot(qmethod,x_it-x_it_old,By,Bx,A,xy,xyp,XX,YY,bound,spc,99); drawnow;
end
%
