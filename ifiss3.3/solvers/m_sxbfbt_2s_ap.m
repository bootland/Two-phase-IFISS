function y = m_sxbfbt_2s_ap(x_it,aparams,mparams)
...
if mparams.domain==1,
   ...
   rp1 = zeros(np,1);
   rp1(minor) = (mparams.Ap(minor,minor))\rp(minor);
   rp2 =  xB*(aparams.F*(xB'*rp1)) + (mparams.Cp2*rp1);
   zp = zeros(np,1);
   zp(minor) = - (mparams.Ap(minor,minor))\rp2(minor);
else 
   zp = mparams.Ap\rp;
   zp = -mparams.Ap \ (xB*(aparams.F*(xB'*zp)) + (mparams.Cp2*zp) );
end 
