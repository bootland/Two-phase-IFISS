function [a,b,bbx,bby,rhor,f,g,l] = stokes_q2q1_two_phase_on_elements_plotting(xy,xyp,mv,mp,tpve,tpde)
%STOKES_Q2Q1_TWO_PHASE_ON_ELEMENTS_PLOTTING Q2-Q1 matrix generator
...
% inds1 = find(tpdve == min(tpdve));
% inds2 = find(tpdve == max(tpdve));
% figure(30), hold on
% plot(x(mv(inds1,9)),y(mv(inds1,9)),'gs','MarkerSize',8,'MarkerFaceColor','g')
% plot(x(mv(inds2,9)),y(mv(inds2,9)),'ys','MarkerSize',8,'MarkerFaceColor','y')
% plot(xy(:,1),xy(:,2),'ro'), drawnow, hold off, pause(1), set(gcf,'Visible','off'), drawnow
ae = zeros(nel,9,9);
le = zeros(nel,9,9);
rhore = zeros(nel,9,9);
bbxe = zeros(nel,9,9);
...
for igpt = 1:nngpt
    ...
    for j = 1:9
        for i = 1:9
            ...
            le(:,i,j)  = le(:,i,j)  + wght*dpsidx(:,i).*dpsidx(:,j).*invjac(:);
            le(:,i,j)  = le(:,i,j)  + wght*dpsidy(:,i).*dpsidy(:,j).*invjac(:);
